...
- **get-results**: Retrieve results from a previously started gadget
...
limited to the specified namespaces otherwise it will be collected for all namespaces.  The gadgets also have use-case-specific parameters that can be passed to customize the data collection. For example, in context of `observe_dns`, 
you can ask it for trace queries for specific domains or nameserver.
...
```
Can you check if any DNS queries are failing in the cluster?
```

```
Can you give me DNS queries taking more than 5ms in the cluster?
```

```
Can you check if any pods are having issues connecting each other in the cluster?
```

```
Can you give me the top 3 pods with highest traffic in the cluster?
```

```
Can you observe system calls for the pod my-pod in the default namespace for few seconds? I want to understand why it is slow.
```
...
- The tool requires Inspektor Gadget to be installed in the cluster. If you are running with `--additional-tools=inspektor-gadget` and `--access-level=readwrite` or more, the MCP server will automatically 
  install Inspektor Gadget (using `inpektor_gadget_deploy_undeploy`) in the cluster otherwise you can follow the steps to install it 
  manually: [Inspektor Gadget Installation](https://learn.microsoft.com/en-us/troubleshoot/azure/azure-kubernetes/logs/capture-system-insights-from-aks#how-to-install-inspektor-gadget-in-an-aks-cluster) or
